...
Referência de estilo inicial do TrustCab, enviada pelo usuário em 14 set 2026. Conteúdo original preservado; o bloco "Tailwind v4" foi omitido por repetir os tokens CSS e não se aplicar ao React Native. As adaptações para o app estão em `CLAUDE.md` na raiz do projeto.
...
### sans-serif — detected in extracted data but not described by AI · `--font-sans-serif`
- **Weights:** 700
- **Sizes:** 17px
- **Line height:** 1
...
### Spacing Scale
11, 13, 15, 16, 17, 21, 22, 23, 31, 34, 38, 52, 53, 59, 68, 119 (px) — tokens `--spacing-{n}`
...
**Role:** Primary action trigger on light backgrounds. Background #000d10, text #ffffff, 1000px border-radius, padding 15px 22px 16px 22px, font 17px weight 700. Used for CTAs that need to feel confident and terminal.

### Ghost White Pill Button
**Role:** Secondary action on dark surfaces. Transparent background, text #ffffff, 1px white border, 1000px border-radius, same 15px 22px padding.

### Circular Icon Button
**Role:** Menu toggle and compact icon affordance. 100% border-radius, background #000d10, white icon, square hit area.

### Featured Clay Card
**Role:** Highlighted solution offering. Hard-edged 0px radius rectangle, background #bc7155, white text, generous padding 53px 59px. The only warm block in the system — reserved for one product card per page to create focal asymmetry.

### Standard White Card
**Role:** Default content container. White background on canvas, 0px radius, no border, no shadow. Relies on surrounding negative space and hairline #d5d3d4 dividers above titles to define its boundary.

### Hero Wordmark
**Role:** Brand mark at extreme scale. Set at 131px weight 700, letter-spacing -2.62px, #000d10, paired with a small ® superscript at ~30% scale. Sits flush-left of the hero viewport.

### Hero Headline
**Role:** Primary hero statement. 60-63px weight 700, letter-spacing -1.26px, #000d10, line-height 1.0. Two-line blocks like 'Beyond / Travel.' with the period as a deliberate typographic stop.

### Section Headline
**Role:** Content section title. 37-52px weight 700, letter-spacing -0.37 to -0.52px, #000d10, four-line blocks max with line-height 1.0.

### Feature Block
**Role:** Sub-section with title and description. Title 23px weight 700 in #000d10, body 18px weight 400 in #8e8e95 with 1.61 line-height, separated from the next block by a 1px #d5d3d4 hairline rule. Arranged in 2-column grids.

### Dark Content Section
**Role:** Full-bleed dark band for contrast content. Background #0f0f1c or #151623, white heading at 30-37px, white body at 18px, content right-aligned in a single column.

### Footer Terminal
**Role:** Page-end navigation and identity. Full-bleed #000d10 background, large white wordmark, nav links in #8e8e95 at 20px, multi-column layout with social and legal rows.

### Top Navigation
**Role:** Primary site navigation. Transparent over hero, sticky or absolute positioned. Items at 20px weight 400, #000d10 on light, with 13px vertical gap between stacked items in mobile.
...
| 3 | Dark Slate | `#151623` | Full-bleed dark content sections |
...
## Imagery
Photography and 3D renders dominate the visual language: a single hero 3D render of a white private jet against a soft gradient sky transitioning from pale blue at top to warm cream at the horizon. Hyper-clean, product-showcase style — no lifestyle context. Additional imagery follows the same product-crop logic: tight, isolated, on pure white. Iconography is minimal and always stroke-based in #000d10.

## Layout
Full-bleed sections alternate between pale sky-tinged hero, white content bands, and dark midnight sections. Content sections are right-aligned single-column (heading + 2×2 feature grid) on white. Navigation is a single transparent row with three text links and a circular menu button. Vertical rhythm is set by 80px section gaps with no visible dividers between most bands — the color shift itself separates them.
...
1. Primary Action Button: #bc7155 background, #ffffff text, 9999px radius, compact pill padding.
2. Featured solution card: background #bc7155, text #ffffff, border-radius 0px, padding 53px 59px, title at 37px weight 700, description at 18px weight 400 line-height 29. Use it once per page.
3. Hero section: pale blue-to-cream gradient sky background, wordmark at 131px weight 700 letter-spacing -2.62px in #000d10 flush-left, headline at 60px weight 700 right-aligned, ghost pill button bottom-right.
4. Feature block grid: 2-column layout on white canvas, 1px solid #d5d3d4 hairline above the title, title 23px weight 700 in #000d10, body 18px weight 400 in #8e8e95 with line-height 29. Gap between columns 80px.
5. Dark content section: full-bleed background #0f0f1c, right-aligned content column at ~50% width, heading 37px weight 700 in #ffffff, body 18px weight 400 in #ffffff with line-height 29.

## Similar Brands
VistaJet, NetJets, Blade, Rimowa, Sonos.
